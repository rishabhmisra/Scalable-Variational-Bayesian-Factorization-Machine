...
//------------------------------------------------------------------------------
	...
	protected:
		...
		void draw_all(DataSubset& train) {
		...
		void draw_w0(double& w0, double& reg, DataSubset& train) {
			// h = 1
			// h^2 = 1
			// \sum e*h = \sum e
			// \sum h^2 = \sum 1
	...
	public:
		virtual void init() {
			...
			std::cout<<"in mcmc learn"<<std::endl;
			...
			//std::cout<<test.data->getNumRows()<<std::endl;
			ifstream test_file("/home/hduser/avijit/rishabh/data/webscopetestfinal_libfm1");
    		uint test_row = 0;
    		while (!test_file.eof()) {
