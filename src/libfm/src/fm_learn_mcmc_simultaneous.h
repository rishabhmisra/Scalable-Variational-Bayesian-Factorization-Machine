			...
			for (uint i = num_complete_iter; i < num_iter; i++) {
				...
				if (task == TASK_REGRESSION) {
					...
					double acc_test_this, acc_test_all, acc_test_all_but5,
						ll_test_this, ll_test_all, ll_test_all_but5,mapatk;
					uint k=5;
					...
					_evaluate_class_map(pred_sum_all, test.target, 1.0/(i+1), acc_test_all, mapatk, k, num_eval_cases);
					//std::cout<<num_eval_cases<<std::endl;
					 //_evaluate_class(pred_sum_all, test.target, 1.0/(i+1), acc_test_all, ll_test_all, num_eval_cases);
					 //_evaluate_class(pred_sum_all_but5, test.target, 1.0/(i-5+1), acc_test_all_but5, ll_test_all_but5, num_eval_cases);

					std::cout << "#Iter=" << std::setw(3) << i << "\tTrain=" << acc_train << "\tTest=" << acc_test_all << "\tMAP@"<<k<<"= " << mapatk << std::endl;
...
//--------------------------------------------------------------------------------------------------------------------------
		void _evaluate_class_map(DVector<double>& pred, DVector<DATA_FLOAT>& target, double normalizer, double& accuracy, double& mapatk,uint k, uint& num_eval_cases)
		{
			...
			tr(test_user_prediction_item,test_user)
			{
				...
				//std::cout<<temp<<"\t"<<count_positive_feedback[user_id]<<"\t"<<average_precision(user_index)<<std::endl;
				user_index++;
//				std::cout<<temp<<"\t"<<count_positive_feedback[user_id]<<"\t"<<average_precision(user_index)<<std::endl;
			}
			mapatk = 0.0;
			...
			//std::cout<<mapatk<<"\t"<<average_precision.dim<<std::endl;
